...
impl House {
    pub fn new(pdv: Shutter, chambre_bas: Shutter, chambre_haut: Shutter) -> Self {
        House { pdv, chambre_bas, chambre_haut, already_wheater_operated: AtomicBool::new(false), already_sunset_operated: AtomicBool::new(false), already_sunrise_operated: AtomicBool::new(false) }
    }

    pub async fn close_all_with_sun(&self, running_loop: Arc<AtomicBool>) {
        while running_loop.load(Ordering::SeqCst) {
            ...
            if sunset_close.is_some() && sunset_close.unwrap().hour() == paris_time.hour() && sunset_close.unwrap().minute() <= paris_time.minute() && !self.already_sunset_operated.load(Ordering::SeqCst) {
                info!("Sunset time reached, closing all shutters");
                ...
                if self.already_sunset_operated.load(Ordering::SeqCst) && paris_time.hour() == 0 && paris_time.minute() <= 9 {
                    ...
                    debug!("Already operated for sunset, reset for next day: {:?} {:?}", sunset_time, paris_time);
                } else {
                    debug!("Not sunset time yet: {:?} {:?} or already operated {:?}", sunset_time, paris_time, self.already_sunset_operated.load(Ordering::SeqCst));
                }
            }
    ...
    pub async fn open_with_sun(&self, running_loop: Arc<AtomicBool>) {
        while running_loop.load(Ordering::SeqCst) {
            ...
            if sunrise_open.is_some() && sunrise_open.unwrap().hour() == paris_time.hour() && sunrise_open.unwrap().minute() <= paris_time.minute() && !self.already_sunrise_operated.load(Ordering::SeqCst) {
                info!("Sunrise time reached, opening all shutters");
                self.pdv.open().await;
                ...
                if self.already_sunrise_operated.load(Ordering::SeqCst) && paris_time.hour() == 0 && paris_time.minute() <= 9 {
                    ...
                    debug!("Already operated for sunrise, reset for next day: {:?} {:?}", sunrise_time, paris_time);
                } else {
                    debug!("Not sunrise time yet: {:?} {:?} or already operated {:?}", sunrise_time, paris_time, self.already_sunrise_operated.load(Ordering::SeqCst));
                }
            }
            tokio::time::sleep(tokio::time::Duration::from_secs(5*60)).await;
    ...
    pub async fn check_weather_and_operate_shutter(&self, weather_response: Arc<Mutex<WeatherData>>,  running_loop: Arc<AtomicBool>) {
       while running_loop.load(Ordering::SeqCst) {
            ...
            if paris_time.hour() >= 10 && paris_time.hour() < 18 && !self.already_wheater_operated.load(Ordering::SeqCst) && weather_response.lock().unwrap().temperature >= 18.0 && weather_response.lock().unwrap().wind_speed < 30.0 && weather_response.lock().unwrap().clouds < 75 {
                info!("Conditions obtenues pour passer les volets de la piece de vie et des chambres du haut en position intermediaire: {:?}", weather_response.lock().unwrap());
                ...
                debug!("Conditions not met for closing mid the shutter: {:?} {:?}", weather_response.lock().unwrap(), self.already_wheater_operated.load(Ordering::SeqCst));
            }

            if paris_time.hour() == 18 && paris_time.minute() <= 9 && self.already_wheater_operated.load(Ordering::SeqCst) {
                debug!("Resetting weather operation flag at 18:00: {:?}", weather_response.lock().unwrap());
